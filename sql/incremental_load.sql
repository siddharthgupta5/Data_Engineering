...
AND email NOT IN (SELECT email FROM dim_users);
...
AND product_id NOT IN (SELECT product_id FROM dim_products);
...
AND transaction_type NOT IN (SELECT transaction_type FROM dim_transaction_types);
...
AND a.transaction_id NOT IN (SELECT transaction_id FROM fact_user_transactions);
